...
public class TA2ASwapper extends QueryBuilder {
	...
	public void testSimpleSwap() {
		...
		A2ASwapper swapper = new A2ASwapper() {
			@Override
			public T2<PSOA2A,PSOA2A> pick(PSOMapping m) {
				...
				T2<PSOA2A,PSOA2A> t2 = Tuples.t((PSOA2A)it.next(), (PSOA2A)it.next());
				return t2;
			}
		};
		...
		// A2A swapping does not change number of mappings
		...
		// check if swapped contains same R2Rs 
		// as the original
		...
		// check if swapped contains swapped A2As
